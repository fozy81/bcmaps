...
Looks good but what about in 3D? Don't worry, `rayshader` package has got your back.
...
<img src="../tools/vignette/all-points.png" align="left"/>
...
<img src="../tools/vignette/scot-points.png" align="left"/>
  
Sum capacity available in each authority 
```{r, eval=FALSE}

capacity <- points %>% group_by(code) %>% 
  summarise(total = sum(capacity)) %>% st_set_geometry(NULL)
...
population <- population %>%  filter(measureType == "count",
                                     age == "all",
                                     gender == "all")
...
<img src="../tools/vignette/bicycle-map.png" align="left"/>
...
select("Local Authority" = local_authority, "Capacity per 100 people" = per_100) %>% 
knitr::kable()
